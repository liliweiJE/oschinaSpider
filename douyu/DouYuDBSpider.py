...
def start(roomid):
		...
		if 'danmudata_{}_{}-{}-{}.db'.format(get_name(roomid), localtime().tm_year, localtime().tm_mon,
			...
			'danmudata_{}_{}-{}-{}.db'.format(get_name(roomid), localtime().tm_year, localtime().tm_mon, localtime().tm_mday))
		else:
			conn = sqlite3.connect(
			'danmudata_{}_{}-{}-{}.db'.format(get_name(roomid), localtime().tm_year, localtime().tm_mon, localtime().tm_mday))
		...
		while True:
			...
			if not data:
				...
				for i in range(0,len(data_more)):
					print(data_more[i].decode(encoding="utf-8"))
					try:
						print("lv:"+level_more[i].decode(encoding="utf-8")+">>>>>>"+uid_more[i].decode(encoding="utf-8")+":"+data_more[i].decode(encoding="utf-8"))
						#conn.execute("INSERT INTO DANMU(level,NAME,danmu) VALUES ({0},'{1}','{2}')".format(level_more[i].decode(),uid_more[i].decode(),data_more[i].decode()))
						#conn.commit()
